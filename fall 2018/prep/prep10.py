...
class BoolOp(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        """
        if self.op == "and":
            return all([v.evaluate() for v in self.values])
        return any([v.evaluate() for v in self.values])
...
class Compare(Expr):
    ...
    def evaluate(self) -> Any:
        ...
        True
        """
        last = self.left.evaluate()
        for op, num in self.comparisons:
            if op == '<=':
                if last > num.evaluate():
                    return False
            else:
                if last >= num.evaluate():
                    return False
