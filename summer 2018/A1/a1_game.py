...
def perform_attack():
    ...
    # Get the next character in the battle _queue, but don't remove them.
    ...
    if next_character.is_valid_action(move_to_make):
        ...
        # (if they still have _sp; otherwise the next call to remove()
        # should skip them)
...
def set_up_game():
    """
    Sets up the battle _queue and characters for the game.
    """
    global P1, P2, BATTLE_QUEUE

    # Create a new battle _queue
    ...
    # The parameters passed in are: their name, the battle _queue and an
